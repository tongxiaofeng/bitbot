...
func (b *BTCC) Setup(exconfig ExchangeAPI) {
	if !exconfig.Enabled {
		b.SetEnabled(false)
	} else {
		b.Enabled = true
		b.AuthenticatedAPISupport = exconfig.AuthenticatedAPISupport
		b.SetAPIKeys(exconfig.APIKey, exconfig.APISecret)
		b.RESTPollingDelay = exconfig.RESTPollingDelay
		b.Verbose = exconfig.Verbose
		b.BaseCurrencies = SplitStrings(exconfig.BaseCurrencies, ",")
		b.AvailablePairs = SplitStrings(exconfig.AvailablePairs, ",")
		b.EnabledPairs = SplitStrings(exconfig.EnabledPairs, ",")
	}
}
...
func (b *BTCC) Run() {
	if b.Verbose {
		log.Printf("%s polling delay: %ds.\n", b.GetName(), b.RESTPollingDelay, " ms.")
