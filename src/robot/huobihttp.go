...
func (h *HUOBI) Setup(exconfig ExchangeAPI) {
	if !exconfig.Enabled {
		h.SetEnabled(false)
	} else {
		h.Enabled = true
		h.AuthenticatedAPISupport = exconfig.AuthenticatedAPISupport
		h.SetAPIKeys(exconfig.APIKey, exconfig.APISecret)
		h.RESTPollingDelay = exconfig.RESTPollingDelay
		h.Verbose = exconfig.Verbose
		h.BaseCurrencies = SplitStrings(exconfig.BaseCurrencies, ",")
		h.AvailablePairs = SplitStrings(exconfig.AvailablePairs, ",")
		h.EnabledPairs = SplitStrings(exconfig.EnabledPairs, ",")
	}
}
...
func (h *HUOBI) Run() {
	if h.Verbose {
		log.Printf("%s polling delay: %ds.\n", h.GetName(), h.RESTPollingDelay, " ms.")
	...
	for h.Enabled {
		...
		time.Sleep(time.Millisecond * h.RESTPollingDelay)
	}
}
